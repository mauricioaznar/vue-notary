...
const lastActivityUpdateCheck =
    window.localStorage.getItem('last_activity_update_check') || moment().format(dateTimeFormat)
        ...
        return function () {
            if (state.lastActivityUpdateCheck) {
                return  state.activities.filter(activity => {
                    const momentLastActivityUpdateCheck = moment(state.lastActivityUpdateCheck)
